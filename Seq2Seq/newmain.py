...
import pickle
import pandas as pd
from dataset import create_dataloader, TextDataLoader, TextDataset
from preprocessing import TextDataSet, limit_unk_vocab
...
def main():
    ...
    # if SAVE:
    #     np.save('all_text.npy', all_text)
    #     np.save('train_text.npy', train_text)
    #     np.save('test_text.npy', test_text)
    #     np.save('train_summary.npy', train_summary)
    #     np.save('test_summary.npy', test_summary)
    ...
    #     train_summary = np.load('train_summary.npy')
    #     test_summary = np.load('test_summary.npy')
    ...
    train_loader = DataLoader(TextDataset(train_text[:subset], train_summary[:subset], transpose=False), batch_size=32)
    test_loader = DataLoader(TextDataset(test_text[:subset], test_summary[:subset], transpose=False), batch_size=32) 
    ...
    if loadzik:
        model = Seq2SeqWithAttention(vecs_enc=global_vectors, idx2word_enc=all_text_model.idx2word, emb_arg_enc=em_sz_enc, 
                                ...
                                activation=activation, pad_idx=all_text_model.word2idx['_pad_']).cpu()
        ...
        history = fit(model, train_loader, val_loader=test_loader, learning_rate=1e-4, epochs=len(tr_ratios), 
                    ...
                    print_period=100, SAVE=True, save_path=os.path.join(OUTPUT_PATH, 'checkpoint.pth.tar'))
    print("Done!\n")    
