...
from dataset import load_data, create_dataloader, save_pickle
...
def fit(model, train_loader, val_loader, opt_fn=None, learning_rate=1e-5, epochs=1, cycle_len=1, metrics=None, 
...
def main():
    ...
    train_text, test_text, train_summary, test_summary = train_test_split(train_text, train_summary, test_size=0.2, random_state=42)
    ...
    test_text = [np.array(t, dtype=np.int64) for t in test_text]
    test_summary = [np.array(s, dtype=np.int64) for s in test_summary]
    
    # Save processed data
    save_pickle(train_text, OUTPUT_PATH, 'train_text.pkl')
    save_pickle(train_summary, OUTPUT_PATH, 'train_summary.pkl')
    save_pickle(test_text, OUTPUT_PATH, 'test_text.pkl')
    save_pickle(test_summary, OUTPUT_PATH, 'test_summary.pkl')
    
    batch_size = 32
    train_loader = create_dataloader(train_text, train_summary, batch_size=batch_size, shuffle=True)
    val_loader = create_dataloader(test_text, test_summary, batch_size=batch_size, shuffle=False)
    ...
    fit(model, train_loader, val_loader, learning_rate=1e-3, epochs=10, cycle_len=1, save=True, save_path=OUTPUT_PATH + '/checkpoint.pth.tar', print_period=100, grad_clip=0.0)
